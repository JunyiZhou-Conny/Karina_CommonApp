...
- If both sides have a square root (or one side does), **square both sides**. Do not stop after looking at one side.
- \(\sqrt{(x-h)^2}=|x-h|\). That is not “\(x\ge h\), so the answer is \(h\).”
- A domain leftover (\(x\ge 2\), \(x\ge 4\)) is a restriction, **not** a solution. Solve, then check.
- “Smallest solution” means the lesser number that actually works. Check both candidates in the original equation.
...
- Vertex form displays a max/min as the outside constant. Factored form does not.
- \(a\cdot b^{x}+c\) displays \(c\), not the \(y\)-intercept (the intercept is \(a+c\)).
- \(a\cdot b^{x+k}\) rewritten as \((ab^{k})\,b^{x}\) displays the intercept as the coefficient \(ab^{k}\).

### Leftover (two items only)
...
**1.** (SPR) The frequency table summarizes a data set.
